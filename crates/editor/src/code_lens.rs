...
const UNRESOLVED_LENS_TITLE: &str = "…";
...
impl Editor {
    pub(super) fn refresh_code_lenses(
        ...
        self.refresh_code_lens_task = cx.spawn(async move |editor, cx| {
            ...
            for (buffer_id, result) in results {
                match result {
                    Ok(Some(actions)) => {
                        let individual_lenses: Vec<(Anchor, Arc<CodeLensItemData>)> = actions
                            .into_iter()
                            .filter_map(|action| {
                                let title: SharedString = match &action.lsp_action {
                                    project::LspAction::CodeLens(lens) => lens
                                        .command
                                        .as_ref()
                                        .map(|cmd| cmd.title.clone().into())
                                        .unwrap_or_else(|| UNRESOLVED_LENS_TITLE.into()),
                                    _ => return None,
                                };
                                ...
                                let item = Arc::new(CodeLensItemData { title, action });
            ...
            editor
                .update(cx, |editor, cx| {
                    ...
                    for (buffer_id, _) in &new_lenses_per_buffer {
                        ...
                        code_lens.lines.remove(buffer_id);
                    }
                    ...
                    for (buffer_id, lines) in new_lenses_per_buffer {
                        if lines.is_empty() {
                            continue;
                        }
                        let code_lens = editor.code_lens.get_or_insert_with(CodeLensState::default);
                        code_lens.lines.insert(buffer_id, lines);
                        editor.rebuild_code_lens_blocks(buffer_id, cx);
                    }
    ...
    fn rebuild_code_lens_blocks(&mut self, buffer_id: BufferId, cx: &mut Context<Self>) {
        let code_lens = match &self.code_lens {
            Some(cl) => cl,
            None => return,
        };
        let Some(lines) = code_lens.lines.get(&buffer_id) else {
            return;
        };
        if lines.is_empty() {
            return;
        ...
        if let Some(old_ids) = self
            .code_lens
            .as_mut()
            .and_then(|cl| cl.block_ids.remove(&buffer_id))
        {
            self.remove_blocks(old_ids.into_iter().collect(), None, cx);
        }
        ...
        let lines = self
            ...
            .get(&buffer_id)
            .unwrap();
        let blocks: Vec<BlockProperties<Anchor>> = lines
            .iter()
            .map(|lens| {
                let render_fn = render_code_lens_line(lens.clone(), editor_handle.clone());
                BlockProperties {
                    placement: BlockPlacement::Above(lens.position),
                    height: Some(1),
    ...
    fn resolve_code_lens(
        &mut self,
        target_item: Arc<CodeLensItemData>,
        cx: &mut Context<Self>,
    ) -> Task<()> {
        if target_item.action.resolved {
            return Task::ready(());
        }
        let Some(project) = self.project.clone() else {
            return Task::ready(());
        ...
        let Some(excerpt_buffer) = buffer.read(cx).as_singleton() else {
            return Task::ready(());
        };
        let lsp_store = project.read(cx).lsp_store();
        let action = target_item.action.clone();

        cx.spawn(async move |editor, cx| {
            let resolve_task = lsp_store.update(cx, |lsp_store, cx| {
                lsp_store.resolve_code_action(&excerpt_buffer, action, cx)
            });
            ...
            match resolve_task.await {
                Ok(resolved_action) => {
                    editor
                        .update(cx, |editor, cx| {
                            let Some(code_lens) = &mut editor.code_lens else {
                                return;
                            };
                            let mut buffer_id_to_rebuild = None;

                            'outer: for (buffer_id, lines) in &mut code_lens.lines {
                                for line in lines {
                                    for item in &mut line.items {
                                        if Arc::ptr_eq(item, &target_item) {
                                            let title = match &resolved_action.lsp_action {
                                                LspAction::CodeLens(lens) => lens
                                                    .command
                                                    .as_ref()
                                                    .map(|cmd| cmd.title.clone().into())
                                                    .unwrap_or_else(|| {
                                                        UNRESOLVED_LENS_TITLE.into()
                                                    }),
                                                _ => UNRESOLVED_LENS_TITLE.into(),
                                            };
                                            *item = Arc::new(CodeLensItemData {
                                                title,
                                                action: resolved_action,
                                            });
                                            buffer_id_to_rebuild = Some(*buffer_id);
                                            break 'outer;
                                        }
                                    }
                                }
                            ...
                            if let Some(buffer_id) = buffer_id_to_rebuild {
                                editor.rebuild_code_lens_blocks(buffer_id, cx);
                                cx.notify();
                            }
                        })
                ...
                Err(error) => {
                    log::warn!("Failed to resolve code lens: {error:#}");
                }
            }
        })
    }

    fn handle_code_lens_click(
        &mut self,
        item: Arc<CodeLensItemData>,
        position: Anchor,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        // Already resolved: try client-side command handling first, fall back to apply.
        if item.action.resolved {
            let task = if let Some(task) =
                try_handle_client_command(&item.action, self, &workspace, window, cx)
            {
                task
            } else {
                let apply_task = project.update(cx, |project, cx| {
                    project.lsp_store().update(cx, |lsp_store, cx| {
                        lsp_store.apply_code_action(
            ...
            if let Some(code_lens) = &mut self.code_lens {
                code_lens.tasks.push(cx.background_spawn(async move {
                    task.await.log_err();
                }));
            }
            return;
        ...
        // Unresolved: go through apply_code_action which resolves + executes,
        // working for both local and remote (SSH) cases.
        let apply_task = project.update(cx, |project, cx| {
            project.lsp_store().update(cx, |lsp_store, cx| {
                lsp_store.apply_code_action(excerpt_buffer.clone(), item.action.clone(), true, cx)
            })
        });
        if let Some(code_lens) = &mut self.code_lens {
            code_lens.tasks.push(cx.background_spawn(async move {
                apply_task.await.log_err();
            }));
        }
    }
...
fn group_lenses_by_row(
    lenses: Vec<(Anchor, Arc<CodeLensItemData>)>,
    snapshot: &MultiBufferSnapshot,
) -> Vec<CodeLensLine> {
    let mut grouped: HashMap<u32, (Anchor, Vec<Arc<CodeLensItemData>>)> = HashMap::default();
...
fn render_code_lens_line(
    ...
    move |cx| {
        ...
        for (i, item) in lens.items.iter().enumerate() {
            ...
            let resolved = item.action.resolved;
            ...
            children.push(
                div()
                    ...
                    .on_hover({
                        ...
                        let item_arc = item_arc.clone();
                        move |hovered, _window, cx| {
                            if !hovered || item_arc.action.resolved {
                                return;
                            }
                            if let Some(editor) = editor_handle.upgrade() {
                                editor.update(cx, |editor, cx| {
                                    let task = editor.resolve_code_lens(item_arc.clone(), cx);
                                    if let Some(code_lens) = &mut editor.code_lens {
                                        code_lens.tasks.push(task);
                                    }
                                });
                            }
                        }
                    ...
                    .on_click({
                        ...
                        move |_event, window, cx| {
                            if let Some(editor) = editor_handle.upgrade() {
                                let item_arc = item_arc.clone();
                                editor.update(cx, |editor, cx| {
                                    editor.handle_code_lens_click(item_arc, position, window, cx);
                                });
                            }
                        }
                    })
                    .when(!resolved, |el| el.italic())
                    .into_any_element(),
            );
        }
...
fn try_schedule_runnable(
    ...
    let runnable = serde_json::from_value::<lsp_ext_command::Runnable>(first_arg.clone()).ok()?;
...
fn try_show_references(
    ...
    let locations = serde_json::from_value::<Vec<lsp::Location>>(arguments[2].clone()).ok()?;
    ...
    Some(cx.spawn_in(window, async move |_editor, cx| {
...
mod tests {
    ...
    use crate::{
        code_lens::UNRESOLVED_LENS_TITLE,
    ...
    async fn test_code_lens_resolve(cx: &mut TestAppContext) {
        ...
        let first_item = cx.editor.read_with(&cx.cx.cx, |editor, _cx| {
            let total_blocks: usize = editor
                .code_lens
                .as_ref()
                .map(|s| s.block_ids.values().map(|v| v.len()).sum())
                .unwrap_or(0);
            assert_eq!(total_blocks, 2, "Should have two code lens blocks");
            ...
            let items: Vec<_> = editor
                ...
                .lines
                .values()
                .flat_map(|lines| lines.iter().flat_map(|line| line.items.clone()))
                .collect();
            assert_eq!(items.len(), 2);
            for item in &items {
                assert_eq!(item.title.as_ref(), UNRESOLVED_LENS_TITLE);
                assert_eq!(item.action.resolved, false);
            }
            items[0].clone()
        });

        let resolve_task = cx
            .update_editor(|editor, _window, cx| editor.resolve_code_lens(first_item.clone(), cx));
        resolve_task.await;
        cx.run_until_parked();
        ...
        cx.editor.read_with(&cx.cx.cx, |editor, _cx| {
            let items: Vec<_> = editor
                ...
                .flat_map(|lines| lines.iter().flat_map(|line| line.items.clone()))
                .collect();
            assert_eq!(items[0].title.as_ref(), "3 references");
            assert_eq!(items[0].action.resolved, true);
        });
    }
}
